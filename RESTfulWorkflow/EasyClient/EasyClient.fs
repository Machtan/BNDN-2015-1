...
//Gets the list of events or the provided events list as a default value if it fails.
let GetEventsList events =    
             ...
             if (eventsResponse.IsSome) then
                (getStatusAndRemovefaulty eventsResponse.Value baseUrl)
             else                
                events 
